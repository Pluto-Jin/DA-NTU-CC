...
parser.add_argument('--data', default='/home/hewei/CrowdCounting/', type=str, metavar='PATH',
                    help='path to dataroot (default: current directory)')
parser.add_argument('--save', default='/home/hewei/temp/', type=str, metavar='PATH',
...
parser.add_argument('--test-mode', default='normal_training',
                    type=str,help='list images to inference (default: none)')
parser.add_argument('--model-type', default='VGG_Decoder', type=str,
...
for file in glob.glob('/export/home/hewei/ntu_random_test/*'):
    file_folder.append('ntu_random_test')
    file_name.append(os.path.basename(file).replace('.png',''))
    
with open(os.path.join(args.data,args.test_mode,test_list[args.test_mode])) as f:
    lines = f.readlines()
for line in lines:
    splited = line.strip().split()
    file_folder.append(splited[0])
    file_name.append(splited[1])
...
for folder,file in zip(file_folder,file_name):
    ...
    d.text((10,10), "Ground Truth:{:.1f}".format(gt),font=font, fill=(255,0,0))   
...
for string in output_str:
    print(string)
with open(os.path.join(args.save,'results.txt'),'w') as output:
