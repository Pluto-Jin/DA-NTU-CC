...
def logger(exp_path, exp_name, work_dir, exception,train_loader, val_loader, resume=False,cfg=None):
#     print('loggger!')
...
#     cfg_file = open('./config_Resnet50_GCC_inducing_CAP.py',"r")  
    cfg_lines = [str(ele) for ele in cfg.items()]
    import socket
    with open(log_file, 'a') as f:
        f.write('Exp:{} in Host:{}\n'.format(exp_name,socket.gethostname()))
        f.write('(Training Batch Size, {})\n(Validation Batch Size, {})\n'.format(train_loader.batch_size,val_loader.batch_size))
        f.write('\n'.join(cfg_lines) + '\n\n')
#         f.write('\n\n')
#     if not resume:
...
def print_summary(log_txt,epoch,exp_name,scores,train_record):
    mae, mse, loss = scores
    print( '='*50 )
    ...
    with open(log_txt, 'a') as f:
        ...
        f.write(str(epoch) + '\n')
        f.write( '    '+ '-'*20+ '\n' )
        f.write( '    [mae %.2f mse %.2f], [val loss %.4f]' % (mae, mse, loss) + '\n')        
        f.write( '    '+ '-'*20 + '\n')
        f.write( '[best] [model: %s] , [mae %.2f], [mse %.2f]' % (train_record['best_model_name'],\
                                                            train_record['best_mae'],\
                                                            train_record['best_mse']) + '\n')
        f.write( '='*50)
...
def print_NTU_summary(log_txt,epoch, scores,train_record):
    mae, mse, loss = scores
...
def update_model(net,optimizer,scheduler,epoch,i_tb,exp_path,exp_name,scores,train_record,each_epoch=False,log_file=None):

    mae, mse, loss = scores

    snapshot_name = 'all_ep_%d_mae_%.2f_mse_%.2f' % (epoch + 1, mae, mse)
...
#         train_record['best_mse'] = mse 
    ...
    if each_epoch:
        torch.save(latest_state,os.path.join(exp_path, exp_name, 'epoch_{}_mae_{:.2f}_mse_{:.2f}_state.pth'.format(epoch, mae, mse)))
