...
class Conv2d_IN(nn.Module):
    def __init__(self, in_channels, out_channels, kernel_size, stride=1, NL='relu', same_padding=False, IN=False, dilation=1):
        super(Conv2d_IN, self).__init__()
        padding = int((kernel_size - 1) // 2) if same_padding else 0
        self.conv = []
        if dilation==1:
            self.conv = nn.Conv2d(in_channels, out_channels, kernel_size, stride, padding=padding, dilation=dilation)
...
#         self.bn = nn.BatchNorm2d(out_channels, affine=True) if bn else None
        ...
        self.bn = nn.InstanceNorm2d(out_channels, affine=True) if IN else None
