...
#         train_list = 'train_list.txt'
        val_list = 'train_list_val_split.txt'
        train_list = 'train_list_train_split.txt'
    ...
    val_set = GCC(cfg_data.DATA_PATH+'/txt_list/'+ val_list, 'test', main_transform=None, img_transform=img_transform, gt_transform=gt_transform)
    val_loader = DataLoader(val_set, batch_size=cfg_data.VAL_BATCH_SIZE, num_workers=8, shuffle=True, drop_last=False)

    test_set = GCC(cfg_data.DATA_PATH+'/txt_list/'+ test_list, 'test', main_transform=None, img_transform=img_transform, gt_transform=gt_transform)
    test_loader = DataLoader(test_set, batch_size=cfg_data.VAL_BATCH_SIZE, num_workers=8, shuffle=False, drop_last=False)
